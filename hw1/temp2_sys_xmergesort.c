...
int getStringTillNextLine(char *data_buf, int *read_output_, char *str_, int *data_buf_offset, struct file *fread, unsigned long long *read_offset){
	...
	//printk("read_output_ %d\n", *read_output_);
	//printk("data_buf_offset %d\n", *data_buf_offset);
	//printk("read_offset %llu\n", *read_offset);
...
int CopyArgsToKernelSpace(struct my_args *args, void *arg){
	int returnValue = 0, i = 0;//, j = 0;
	struct my_args *source = (struct my_args*)arg;
...
*/	
	...
	while(read_output_1 > 0 && read_output_2 > 0){
		...
		while(data_buf_offset1  < read_output_1 && data_buf_offset2 < read_output_2){//till the data buf exausts){
				if(temp1 > 50)
					break;
				temp1++;
				//string compare
