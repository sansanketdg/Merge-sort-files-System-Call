...
int copyToOutputBuf(char *output_buf, int actual_out_buf_offset, char *str_file, int startIndex, int str_len, struct file *tempFilePtr, unsigned long long *output_buf_offset){
	int i, write_output;
...
asmlinkage long xmergesort(void *arg)
{
	...
	int temp1 = 0, str_len1 = 0, str_len2 = 0, actual_out_buf_offset = 0;
...
//		return -EDOM; 
	...
	*/
	tempOutFile = kmalloc(strlen(args->output_filename) + 5,
					GFP_KERNEL);
	...
	f_temp_write_return = file_open(tempOutFile, O_RDWR | O_CREAT, S_IWUSR | S_IRUSR, &tempFilePtr);
	...
	*/
	...
	lock_rename(tempFilePtr->f_path.dentry->d_parent,
				outFilePtr->f_path.dentry->d_parent);

	oldfs = get_fs();
	set_fs(KERNEL_DS);

	returnValue = vfs_rename(tempFilePtr->f_path.dentry->d_parent->d_inode,
...
label_partial_fail:
	...
	*/
	oldfs = get_fs();
	set_fs(KERNEL_DS);
	retval = vfs_unlink(tempFilePtr->f_path.dentry->d_parent->d_inode,
...
//clean_tempOutFilePtr:
		//file_close(&tempFilePtr);
		//printk(KERN_ALERT "%s temp file closed\n", args->output_filename);
clean_all:
		kfree(data_buf_file1);
		kfree(data_buf_file2);
		kfree(str_file1);
