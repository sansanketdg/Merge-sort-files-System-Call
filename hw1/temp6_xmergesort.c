...
/* Flag to allow printing debug statements. Please set this to 0 and
 rebuild this module to prevent printing debug messages
*/
#define DEBUGON		1
/* Flag to allow printing
*/
#define BUFFER_SIZE PAGE_SIZE/10
...
*/
int getStringTillNextLine(char *data_buf, int *read_output_, char *str_,
 int *data_buf_offset, struct file *fread, unsigned long long *read_offset)
{
	...
	*data_buf_offset = i+1;
	if (i == *read_output_) {
		...
		*data_buf_offset = 0;
		i = 0;
		*read_output_ = file_read(fread, *read_offset, data_buf, 
			BUFFER_SIZE);
		if (*read_output_ > 0) {
...
*/
int copyToOutputBuf(char *output_buf, int actual_out_buf_offset, 
	char *str_file, int str_len, struct file *fwrite, 
	unsigned long long *output_buf_offset)
{
	int i, write_output;
	int startIndex = 0;
...
int ValidateArgument(void *arg){
	...
	int returnValue, i;
	struct kstat inputFileStat[2], outputFileStat;
	...
	for(i = 0; i < args->input_filecount; i++){
		returnValue = vfs_stat(args->input_filenames[i], &inputFileStat[i]);
		...
		if (returnValue != -2 && !(S_ISREG(inputFileStat[i].mode))) {
			printk(KERN_ALERT "Input-file%d is a directory \n", i+1);
			goto label_invalid_arg;
		}

		/* The input and output file should not be same */
		if ((inputFileStat[i].dev == outputFileStat.dev)
			&& (inputFileStat[i].ino == outputFileStat.ino)) {
				printk(KERN_ALERT "Error: I/p-%d and O/p file are same \n", i+1);
				returnValue = -EINVAL;
				goto label_invalid_arg;
		}
	}
	if ((inputFileStat[0].dev == inputFileStat[1].dev)
			&& (inputFileStat[0].ino == inputFileStat[1].ino)) {
				printk(KERN_ALERT "Error: I/p-0 and I/p-1 file are same \n");
				returnValue = -EINVAL;
				goto label_invalid_arg;
	}
					...
					if(strcasecmp(str_file1, str_file2) <= 0){
						...
						if((args->flags & OPTION_DUPLICATES_ALLOWED) == OPTION_DUPLICATES_ALLOWED){
							if(strcasecmp(str_file1, last_out_buf_str) >= 0){
								...
								actual_out_buf_offset = copyToOutputBuf(output_buf, actual_out_buf_offset, str_file1, str_len1, tempFilePtr, &output_buf_offset);
						...
						if((args->flags & OPTION_DUPLICATES_ALLOWED) == OPTION_DUPLICATES_ALLOWED){
							...
							if(strcasecmp(str_file2, last_out_buf_str) >= 0){
								...
								actual_out_buf_offset = copyToOutputBuf(output_buf, actual_out_buf_offset, str_file2, str_len2, tempFilePtr, &output_buf_offset);
								no_of_sorted_elements++;
								memset(last_out_buf_str, 0, BUFFER_SIZE);
